...
namespace CryptoClients.Net
{
    ...
    public partial class ExchangeSocketClient
    {
            ...
            CancellationToken ct = default)
        {
            ...
            var tasks = clients.Where(x => x.SubscribePositionOptions.Supported).Select(x => Task.Run(async () =>
            {
                ...
                return new ExchangeResult<UpdateSubscription>(x.Exchange, await x.SubscribeToPositionUpdatesAsync(request with { ListenKey = listenKey }, handler, ct).ConfigureAwait(false));
            }));
